...
def redact_file_content(content):
    # Preserve timestamps by temporarily replacing them
    timestamps = re.findall(TIMESTAMP_PATTERN, content)
    for i, ts in enumerate(timestamps):
        placeholder = f"__TIMESTAMP_{i}__"
        content = content.replace(ts, placeholder)
    ...
    for i, ts in enumerate(timestamps):
        placeholder = f"__TIMESTAMP_{i}__"
        content = content.replace(placeholder, ts)
