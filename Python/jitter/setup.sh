...
# Step 4: Add MQ drop mark rule to custom chain
sudo iptables -t mangle -C $CHAIN_NAME -p tcp --dport $PORT -j MARK --set-mark $TABLE_ID 2>/dev/null || \
sudo iptables -t mangle -A $CHAIN_NAME -p tcp --dport $PORT -j MARK --set-mark $TABLE_ID
...
echo "[OK] MQ blackhole simulation setup complete."
